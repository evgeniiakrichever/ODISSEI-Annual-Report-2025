...
// Each workstream gets its own page with NO .flow class
// so page-break-inside: avoid is enforced — no mid-text breaks.
function wsPage(ws) {
  return `
  <div class="content-page ws-page">
    <div class="ws-label">${ws.num}</div>
    <h2 class="ws-title">${ws.title}</h2>
    <p class="ws-lead">${ws.lead}</p>
    <div class="ws-body">
      ${ws.achievements.map(a => `<p>${a}</p>`).join('\n      ')}
    </div>
...
p { margin-bottom: 0.55em; } p:last-child { margin-bottom: 0; }
ul { list-style: none; }
...
/* Pages that may naturally flow across pages (non-workstream) */
.content-page.flow { page-break-inside: auto; }

/* Workstream pages: strictly no page breaks inside */
.ws-page {
  width: 210mm;
  height: 297mm;
...
.section-num { font-family: var(--serif); font-size: 55pt; font-weight: 700; color: var(--light); position: absolute; top: 10mm; right: 14mm; line-height: 1; pointer-events: none; }
...
/* Rule */
.rule { border: none; border-top: 1.5px solid var(--rule); margin: 4mm 0; }
...
.board-panel { padding: 3.5mm; background: var(--light); border-radius: 2px; page-break-inside: avoid; }
...
/* Workstreams */
.ws-label { font-size: 7pt; font-weight: 700; letter-spacing: 0.15em; text-transform: uppercase; color: var(--teal); margin-bottom: 1.5mm; }
.ws-title { font-family: var(--serif); font-size: 18pt; font-weight: 700; color: var(--ink); line-height: 1.15; margin-bottom: 2.5mm; }
...
.ws-body { flex: 1; margin-bottom: 4mm; overflow: hidden; }
.ws-body p { font-size: 8pt; line-height: 1.65; color: var(--ink); margin-bottom: 2.5mm; }
.ws-body p:last-child { margin-bottom: 0; }
.budget-box { background: var(--dark); padding: 4mm; border-radius: 2px; flex-shrink: 0; }
.budget-header { display: flex; justify-content: space-between; align-items: baseline; margin-bottom: 0.5mm; }
...
.budget-sublabel { font-size: 6.5pt; color: rgba(255,255,255,0.4); margin-bottom: 2mm; }
.budget-table { width: 100%; border-collapse: collapse; }
.budget-table tr { border-top: 1px solid rgba(255,255,255,0.08); }
.budget-table td { padding: 1.2mm 0; font-size: 7pt; color: rgba(255,255,255,0.75); }
...
.paper-card { border: 1px solid var(--rule); padding: 4mm; border-radius: 2px; page-break-inside: avoid; }
...
.testi-card { background: var(--light); padding: 4mm; border-radius: 2px; page-break-inside: avoid; }
...
.kpi-cell { padding: 3.5mm; border: 1px solid var(--rule); border-radius: 2px; page-break-inside: avoid; }
...
<!-- WORKSTREAMS — each on its own locked page, no breaks -->
